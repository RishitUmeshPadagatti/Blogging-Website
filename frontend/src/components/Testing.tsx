...
const Tabs: React.FC<TabProps> = ({ labels }) => {
  ...
  return (
    <div className="relative flex items-center border border-black w-[500px]">
      {/* Left Arrow */}
      {showLeftArrow && (
        <button
          onClick={() => {
      ...
      >
        {labels.map((label, index) => (
          <div
            key={index}
            className={`inline-block px-4 py-2 cursor-pointer ${index === 0 ? "font-bold text-black" : "font-medium text-gray-600"
              }`}
          >
            {label}
          </div>
